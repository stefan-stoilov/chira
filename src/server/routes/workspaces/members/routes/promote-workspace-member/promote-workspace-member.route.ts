...
export const promoteWorkspaceMemberRoute = createRoute({
  method: "patch",
  path: "/api/workspaces/{id}/members/{user-id}/promote",
  tags,
  middleware: [sessionMiddleware, isMemberMiddleware] as const,
  request: { params: workspaceMemberParamsSchema },
  responses: {
    [http.OK]: jsonContent(createSuccessSchema(), "Promote Member"),
...
export type PromoteWorkspaceMemberRoute = typeof promoteWorkspaceMemberRoute;
